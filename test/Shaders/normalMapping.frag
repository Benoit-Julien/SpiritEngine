...
LightInfos CalcLightInformations(vec3 lightPosition, vec4 normal) {
    LightInfos infos;
    ...
    infos.L = normalize(tangentMatrix * infos.L);
    infos.N = normal.xyz;
    infos.V = normalize(tangentMatrix * -fPosition);
...
vec4 CalcPointColor(Light light, vec4 texColor, vec4 normal) {
    LightInfos infos = CalcLightInformations(light.position, normal);

    vec3 diffuse = texColor.xyz * light.diffuse * infos.dotNL * light.intensity * infos.attenuation;
    vec3 specular = material.specular * light.specular * infos.pf * light.intensity * infos.attenuation;

    return vec4(diffuse + specular, 1.0);
}

vec4 CalcSpotColor(SpotLight light, vec4 texColor, vec4 normal) {
    LightInfos infos = CalcLightInformations(light.position, normal);
    ...
    vec3 diffuse = texColor.xyz * light.diffuse * light.intensity * infos.dotNL * attenuation;
...
void main() {
    ...
    vec4 normal = texture(material.normalTex, texCoord);

    for (int i = 0; i < lightNumber; i++) {
        result += CalcPointColor(lights[i], texColor, normal);
    }
    for (int i = 0; i < spotNumber; i++) {
        result += CalcSpotColor(spotLights[i], texColor, normal);
    }
